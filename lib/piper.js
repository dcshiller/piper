...
const submitScore = function (e) {
  ...
  updateHighScore(score, initials);
  toggleVisibility ("newHighScorePanel");
  toggleVisibility ("highScoresPanel");
};

const updateHighScore = function (score, initials) {
  $(".highscore").last().destroy();
  for(let i = 0; i < 5; i++){
    if(window.highscores[i] < score){
      window.highscores = window.highscores.slice(0,i).push(`${score} + ${initials}`).concat(window.highscores.slice(i));
      $(`.highscore:nth-child(${i})`).insertBefore($(`<li>${score} ${initials}</li>`));
      break;
    }
  }
};
