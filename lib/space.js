...
Space.prototype.burst = function (direction) {
  let spaceCanvas = this.getCanvas();
  let ctx = spaceCanvas.getContext('2d');
  let correctedDirection = direction;
  let rotations = 1
  while ( correctedDirection != "top") {
    correctedDirection = CLOCKWISE[correctedDirection]
    rotations++
  }
  ctx.translate(25,25);
  ctx.rotate((90*rotations)*Math.PI/180);
  ctx.translate(-25,-25);
  ctx.beginPath();
  ctx.moveTo(25,10);
  ctx.lineTo(38,10);
